...
function DraggableIngredient({ ingredient }) {
  ...
  return (
    <button
      ...
      {...listeners}
      {...attributes}
      className={`relative px-6 py-4 rounded-xl border border-zinc-700 bg-zinc-800/80 backdrop-blur-md shadow-2xl text-lg font-bold transition-colors cursor-grab active:cursor-grabbing
        ${isDragging ? 'opacity-80 scale-110 z-50 ring-2 ring-orange-500' : 'hover:scale-105 hover:bg-zinc-700'}
      `}
    >
      {ingredient.name}
    </button>
...
export default function BurgerStage({ stage, index }) {
  ...
  const handleDragEnd = (event) => {
    ...
    if (over && over.id === `drop-${stage.id}`) {
      if (active.data.current.type === 'correct') {
        setCompleted(true);
        setErrorObj(null);
        // Dispatch event for BurgerStack to catch
  ...
  return (
    <section ref={stageRef} className="relative w-full min-h-screen py-32 md:py-48 flex items-center justify-start border-b border-zinc-900 border-dashed px-4 md:px-16">
        ...
        <DndContext onDragEnd={handleDragEnd}>
          ...
          {!completed && (
            <div
              ...
              `}
            >
              <span className="font-heading text-2xl text-zinc-600 font-black uppercase tracking-widest pointer-events-none">
